-- Update get_public_dashboard_stats to include branding fields
...
BEGIN
  ...
  -- Get site stats with branding fields
  SELECT json_build_object(
    'site_name', s.name,
    'brand_color', s.brand_color,
    'brand_logo_url', s.brand_logo_url,
    'custom_css', s.custom_css,
    'remove_branding', s.remove_branding,
    'title', _config.title,
    ...
    'timeseries', (
      ...
      FROM (
        ...
        FROM events
        WHERE site_id = _site_id AND created_at >= _start_date AND created_at < _end_date + interval '1 day'
        GROUP BY 1
        ORDER BY 1
      ) t
    )
